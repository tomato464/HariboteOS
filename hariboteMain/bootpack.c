...
int keywin_on(struct SHEET *key_win, struct SHEET *sht_win, int cursor_c);
int keywin_off(struct SHEET *keywin, struct SHEET *sht_win, int cursor_c, int cursor_x);
...
int keywin_on(struct SHEET *key_win, struct SHEET *sht_win, int cur_c)
{
	...
	if(key_win == sht_win){
		cur_c = COL8_000000;//カーゾルを出す
	} else {
		if((key_win->flags & 0x20) != 0){//コンソールのカーソルON
...
int keywin_off(struct SHEET *key_win, struct SHEET *sht_win, int cur_c, int cur_x)
{
	...
	if(key_win == sht_win){
		...
		if((key_win->flags & 0x20) != 0){//コンソールのカーソルOFF
