...
void console_task(struct SHEET *sheet, int memtotal)
{
	...
	if(nihongo[4096] != 0xff){//nihongoが読み取れたか
		task->langmode = 1;
	}else{
		task->langmode = 0;
	}
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if(appsiz >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00){
			...
			dathrb	= *((int *)( p + 0x0014));//dataの入った番地
			...
			start_app(0x1b, 0 * 8 + 4, esp, 1 * 8 + 4, &(task->tss.esp0));//アプリ終了時はこの下から再開される
			...
			for(i = 0; i < 8; i++){//openしたままのファイルを閉じる
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax)
{
	...
	struct SHTCTL *shtctl = (struct SHTCTL *) *((int *) 0x0fe4);//haribote.cでそのように記憶させている。
	int *reg = &eax + 1, i;
	//reg[0]~reg[7] = edi ~ eax;
	struct SHEET *sht;
	...
	if (edx == 1) {
		...
		*/
		...
		ebp　文字列
		*/
		sht = (struct SHEET *) (ebx & 0xfffffffe);//偶数にしている
		putfonts8_asc(sht->buf, sht->bxsize, esi, edi, eax, (char *)ebp + ds_base);
		if((ebx & 1) == 0){//ebxが偶数ならrefreshする。
		...
		if(eax == 0){
			...
			i = 1193180000 / eax;//eax mHzの出力となる
			io_out8(0x43, 0xb6);
			io_out8(0x42, (i & 0xff));
			io_out8(0x42, (i >> 8));
