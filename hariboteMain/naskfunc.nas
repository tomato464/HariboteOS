...
_asm_hrb_api:
		...
		CMP		EAX,0	; EAXが0でなければアプリ終了,0なら続行
		JNE		end_app
		ADD		ESP,32
		POPAD
		POP		ES
...
_start_app:		; void start_app(int eip, int cs, int esp, int ds, int *tss_esp0);
		...
		MOV		EBP,[ESP+52]	; 次の引数、tss.esp0の番地
...
;far-callできないのでOS側からfar-returnでapp側に行く。その準備
		OR		ECX,3	; app用のセグメント番号に3をorする
		OR		EBX,3	;
		PUSH	EBX		; appのSS
		PUSH	EDX		; appのESP
		PUSH	ECX		;
